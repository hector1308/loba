...
func _ready():
	...
	# 2. Pintar el mazo visualmente
	actualizar_mazo_visual()
	
	# 3. Desbloquear botones
...
func configurar_interfaz_visual():
	...
	# 4. MANO JUGADOR (Z-INDEX 50 PARA QUE SIEMPRE FLOTE SOBRE LA MESA)
...
func actualizar_mazo_visual():
	...
	if mazo.size() > 0:
		...
		# Espiamos la última carta del mazo (la próxima en salir) para ver su color
		var proxima_carta = mazo.back()
		var color_reverso = proxima_carta.color # Será "blue" o "red"
		
		var carta_dorso = ESCENA_CARTA.instantiate()
		# Usamos el color real de la próxima carta
